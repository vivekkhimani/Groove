...
def parse_user_favorites(access_token, id_list):
    ...
    audio_features = requests.get(full_link, headers=headers, timeout=60)
...
def make_recommendations(access_token, id_list, predictions, num_recommendations):
    ...
    track_details = requests.get(full_link, headers=headers, timeout=60)
    ...
    while len(final_predicted_ids) < num_recommendations:
        ...
        track_det = requests.get(link, headers=headers, timeout=60)
...
def get_playlist():
    ...
    headers = {'Authorization': 'Bearer ' + access_token}
    playlist_data = requests.get('https://api.spotify.com/v1/me/playlists', headers=headers, timeout=60)
...
def top_songs():
    access_token = request.args.get("access_token")
    headers = {'Authorization': 'Bearer ' + access_token}
    top_songs_data = requests.get(' https://api.spotify.com/v1/me/top/tracks?time_range=medium_term&limit=20&offset=5', headers=headers, timeout=60)
...
def playlist():
    ...
    playlist_data = requests.get(full_link, headers=headers, timeout=60);
    playlist_json = playlist_data.json()
    id_list = list()
    for items in playlist_json['items']:
...
def recently_played():
    ...
    recently_played_data = requests.get(spotify_req_url, headers=headers, timeout=60)
...
def saved_tracks():
    access_token = request.args.get("access_token")
    headers = {'Authorization': 'Bearer ' + access_token}
    saved_tracks_data = requests.get('https://api.spotify.com/v1/me/tracks?market=US&limit=20&offset=5', headers=headers, timeout=60)
